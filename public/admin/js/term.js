$(document).ready(function() {
    loadDataTableForTerm();
});
function loadDataTableForTerm(){
  table = $('#term-details-table').DataTable({
      processing: true,
      serverSide: true,
      ajax: $('#route-for-user').val()+'/term/show',
      columns: [
          {data: 'DT_RowIndex', orderable: false, searchable: false},
          {data: 'name', orderable: false, searchable: false},
          {data: null,
              render:function(row){
                 return  `<div class="action-buttons text-center">	<a class="u-link mr-2 primary-btn small tr-bg"  title="Edit"  href="`+$('#route-for-user').val()+`/term/`+row.id+`/edit">
                      <i class="fa fa-eye"></i> Edit
                  </a>
                  <a class="u-link mr-2 primary-btn small tr-bg" href="#"  title="Delete"  onclick="deleteTerm(`+row.id+`)">
...
$('#term-edit-form').validate({
  ...
  submitHandler: function(form,event) {
      ...
      var current_btn_text=submitButton.html();
      button_loading_text = 'Saving...';
      var term_id = $(form).find('input[name=term_id]').val();
      // Create
      $.ajax({
          type: "POST",
          url: $('#route-for-user').val()+'/term/'+term_id,
          contentType: false,
          processData: false,
          data: formData,
          cache: false,
...
function deleteTerm(id){
  ...
  swal({
  title: "Confirm?",
  text: "Are you sure, do yo want to delete the term ?",
  type: "warning",
  ...
  cancelButtonText: "No, cancel it!"
      ...
      if (isConfirm) {
          $.ajax({
              type: "DELETE",
              url: $('#route-for-user').val()+'/term/'+id,
              data:
                  {
                      'id':id,
                  },
